...
**概念**：SYN 攻击就是客户端在短时间内伪造大量不存在的 IP 地址，并向服务器不端的发送大量的 SYN 报文，服务端在接收到有就会回复确认包，等待客户端的确认，但是由于这个 IP地址 是不存在的，因此 服务端会不断的发送包，直到超时，而这些伪造的 SYN 包，长时间的占用未连接队列，导致正常的 SYN包因为队列满了，而被丢弃，从而引起网络拥塞甚至系统瘫痪。

**检测**： 当你看到你的服务器中有大量的半连接状态的时，特别是 IP地址是随机的，基本都是 SYN 攻击；

在 Linux/Unix 上 利用 netstats 命令来检测 
